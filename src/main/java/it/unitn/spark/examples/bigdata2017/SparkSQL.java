...
public class SparkSQL {
  ...
  public static void main(String[] args) {
    ...
    Dataset<Person> javaBeanDS = spark
        .createDataset(Collections.singletonList(person), personEncoder);
    javaBeanDS.show();
    ...
    for (Person p : peopleRDD.collect()) {
      System.out.println(
          p.getName() + "\t" + p.getAge() + "\t" + p.getAddress().getCity() + "\t" + p.getAddress()
              .getState());
    }
    ...
    Dataset<Row> teenagersDF = spark
        .sql("SELECT name FROM people_third WHERE age BETWEEN 13 AND 19");
...
// The schema is encoded in a string
    ...
    Dataset<Row> peopleDF2 =
        spark.read().format("json").load("files/people.json");
    peopleDF2.select("name", "age").write().format("parquet").save("files/namesAndAges.parquet");


    Dataset<Row> directSQLDF =
        spark.sql("SELECT * FROM parquet.`files/users.parquet`");
