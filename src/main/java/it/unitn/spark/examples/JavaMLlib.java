...
public final class JavaMLlib {

	@SuppressWarnings("serial")
	public static void main(String[] args) {
		SparkConf sparkConf = new SparkConf().setAppName("JavaBookExample").setMaster("local[2]");
		...
		// Create a HashingTF instance to map email text to vectors of 100
		// features.
		final HashingTF tf = new HashingTF(100);

		// Each email is split into words, and each word is mapped to one
		// feature.
		// Create LabeledPoint datasets for positive (spam) and negative (ham)
		// examples.
		...
		trainingData.cache(); // Cache data since Logistic Regression is an
								// iterative algorithm.
		...
		// First apply the same HashingTF feature transformation used on the
		// training data.
		Vector posTestExample = tf.transform(Arrays.asList("O M G GET cheap stuff by sending money to ...".split(" ")));
		Vector negTestExample = tf
				.transform(Arrays.asList("Hi Dad, I started studying Spark the other ...".split(" ")));
